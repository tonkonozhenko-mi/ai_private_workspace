...
- **Group screens now use a wide display instead of being stretched across it.** A group was the one project view that never got the wide-screen treatment: on a large monitor its title ran to one edge, its buttons to the other, and a metre of nothing sat in between. It is framed now, like every other screen. Extra width becomes more repository cards per row rather than wider cards; a group conversation is capped and centred at the same measure a single project's Ask uses, because an answer two thousand pixels wide is one your eye loses its place in; and the notes block keeps its heading, its button and its field within reach of each other.
