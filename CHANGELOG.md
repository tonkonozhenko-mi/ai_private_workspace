...
- **The wording you type into a skill is the wording that gets saved.** Editing a skill's guidance and pressing Save appeared to work, said “Saved”, and changed nothing: the panel filed every edit under a single key that nothing downstream reads, so the request left carrying the old text and the old text came back. The same block also showed a picker that opened on the skill currently in use, which made it look like the control that chooses one — it only chooses which skill to edit. It now states which skill answers actually use, calls itself “Skill to edit”, and its button says “Save wording”, because that is all it saves. Which skill is active still follows the project's role.
...
- **Ask for the answer in English and you get English.** A question written in Russian asked for onboarding documentation "all of it in English"; the answer came back in Ukrainian — the language of the wiki it had read. The prompt had never mentioned language at all, so the documents decided. There is a rule now, at the end of the prompt where a small model remembers it best, in the order a person would expect: what you asked for in this question wins; failing that, the preference saved in About you; failing that, the language you wrote the question in. Never the language of the files. It survives a typo, too — "должно біть на английском" asks just as clearly as the correctly spelled version, and a rule a slip can switch off is not a rule you can rely on.
