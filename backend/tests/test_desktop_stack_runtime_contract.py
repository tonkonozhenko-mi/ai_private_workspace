...
def test_desktop_stack_runtime_contract_locks_open_source_cross_platform_direction() -> None:
    ...
    assert {"tauri", "react-vite", "fastapi", "sqlite-qdrant-ollama", "pyinstaller-first"}.issubset(
        component_ids
    )
...
def test_desktop_stack_runtime_contract_preserves_safety_rules() -> None:
    ...
    assert any(
        "scan, index, rebuild, MCP, Agent, or model downloads" in rule
        for rule in payload["safety_rules"]
    )
    assert any(
        "only the staged app-owned backend runtime" in rule for rule in payload["staging_contract"]
    )
    assert any(
        command["command"] == "scripts/check_desktop_stack_contract.sh"
