...
- **A gentle first-run orientation so a new user knows where to start.** After first-time setup, a short guided tour dims the screen and spotlights each section in turn — Home, Intelligence, Ask, Models, Settings — with a one-line caption explaining what it's for, so you learn where everything is and what to click. It runs once, is fully skippable (or step through with the arrow keys), and can be replayed any time from the "Take a quick tour" link. Home also shows a small, dismissible "Start here" card with three steps — get the local AI ready, ask your first question, explore your project's map — each a real button that takes you there and checks itself off as you go; it disappears once you're set up. And the first time you open a section, a one-line caption explains it, then quietly goes away. Nothing blocks you — the app teaches in place and gets out of the way.
