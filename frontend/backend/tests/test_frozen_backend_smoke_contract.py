...
def test_frozen_backend_smoke_contract_is_manual_and_safe():
    ...
    assert any(command["command"] == "scripts/smoke_frozen_backend_runtime.sh" for command in payload["validation_commands"])
    ...
    assert any("Frontend and React still cannot execute shell commands" in rule for rule in payload["safety_rules"])
