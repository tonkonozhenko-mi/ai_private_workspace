...
def test_desktop_runtime_preflight_endpoint_is_read_only_and_safe() -> None:
    ...
    assert any(
        command["command"] == "scripts/check_desktop_runtime_preflight.sh"
        for command in payload["validation_commands"]
    )
    assert any("read-only" in rule.lower() for rule in payload["safety_rules"])
    assert any(
        "Frontend can display and copy commands only" in rule for rule in payload["safety_rules"]
    )
...
def test_desktop_runtime_preflight_documents_blockers_and_pass_criteria() -> None:
    ...
    assert any(
        "Missing backend/app/main.py" in condition for condition in payload["fail_fast_conditions"]
    )
