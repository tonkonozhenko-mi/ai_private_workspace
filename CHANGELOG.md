...
- **A gentle first-run orientation so a new user knows where to start.** The first time you open a project, a short guided tour dims the screen and spotlights each part in turn — the projects button (which is tucked away by default), then Home, Intelligence, Ask, Models and Settings — with a one-line caption explaining what it's for, so you learn where everything is and what to click. It runs once, is fully skippable (or step through with the arrow keys), and can be replayed any time from the command palette's "Take a quick tour". The first time you open a section, a one-line caption also explains it, then quietly goes away. Nothing blocks you — the app teaches in place and gets out of the way.
