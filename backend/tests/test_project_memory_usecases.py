...
from app.core.use_cases.manage_project_memory import AddMemoryInput, AddMemoryUseCase, ListMemoryUseCase, DeleteMemoryUseCase
from app.core.use_cases.build_project_handbook import BuildHandbookInput, BuildProjectHandbookUseCase
from app.core.use_cases.compose_project_context import ComposeProjectContextUseCase


def _graph():
    return ProjectGraph(workspace_id="w1", entities=[
        ProjectEntity(id="cloud_service:aws-lambda", type=EntityType.CLOUD_SERVICE, name="AWS · Lambda", analyzer="terraform"),
        ProjectEntity(id="environment:prd", type=EntityType.ENVIRONMENT, name="prod", analyzer="terraform"),
    ])


def test_add_list_delete():
    repo = InMemoryProjectMemoryRepository()
    item = AddMemoryUseCase(repo).execute(AddMemoryInput(workspace_id="w1", text="prod is called prd", kind=MemoryKind.CORRECTION))
...
def test_handbook_singleton_replaced():
    ...
    graphs = InMemoryProjectGraphRepository(); graphs.save_graph(_graph())
...
def test_compose_context_combines_handbook_memory_graph():
    ...
    AddMemoryUseCase(mem).execute(AddMemoryInput(workspace_id="w1", text="lambda timeout is 30s", kind=MemoryKind.FACT, pinned=True))
