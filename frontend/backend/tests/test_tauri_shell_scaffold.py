...
def test_tauri_shell_scaffold_endpoint_is_read_only_and_safe() -> None:
    ...
    assert any("Frontend React code still never executes shell commands" in rule for rule in payload["safety_rules"])
    assert any("Model downloads remain backend-side" in rule for rule in payload["safety_rules"])
    assert any(file["path"] == "frontend/src-tauri/tauri.conf.json" for file in payload["generated_files"])
