...
def test_pyinstaller_backend_runtime_contract_exposes_frozen_runtime_poc() -> None:
    ...
    assert any(item["id"] == "frozen-manifest" and item["status"] in {"ready_after_command", "ok"} for item in body["items"])
    assert any("open-source" in rule for rule in body["runtime_contract"])
    assert any("Frontend still cannot execute shell commands" in rule for rule in body["safety_rules"])
    assert any(command["command"] == "scripts/check_pyinstaller_backend_runtime.sh" for command in body["validation_commands"])
