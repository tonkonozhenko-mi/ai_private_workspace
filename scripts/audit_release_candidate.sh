...
for path in backend frontend docs scripts pytest.ini .gitignore; do
...
if find . -path './.git' -prune -o \( -name '*.db' -o -name '*.sqlite' \) -print | grep -q .; then
  fail "database files found in source tree; remove them from release archive"
else
  pass "no *.db or *.sqlite files found"
fi

for doc in docs/START_HERE.md docs/ROADMAP.md docs/API_INVENTORY.md docs/PROJECT_CHECKPOINT.md docs/DESKTOP_PACKAGING_DESIGN_LOCK.md docs/WINDOWS_PACKAGING_FOUNDATION.md; do
  if [ -f "$doc" ]; then pass "release doc exists: $doc"; else fail "missing release doc: $doc"; fi
done

for script in scripts/apply_generated_update.sh scripts/package_macos_app_foundation.sh scripts/prepare_tauri_shell_scaffold.sh scripts/prepare_windows_packaging_foundation.sh; do
  if [ -f "$script" ]; then
