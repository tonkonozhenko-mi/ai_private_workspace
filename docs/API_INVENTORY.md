...
| `POST /models/model-search` | Find downloadable GGUF models by name, so nobody needs to know a Hugging Face repository id. Read-only search; a network failure returns an explanatory message rather than an error, and the paste-a-repository path keeps working offline. | No | No | Hugging Face search API | Model setup (llama.cpp) |
| `POST /models/ollama-pulls` | Download any Ollama model through the local daemon's streaming pull API. Deliberately not routed through the command-proposal machinery: no shell command is run, so the execution permission and the catalog allowlist — which exist to constrain shell execution — do not apply. See `backend/app/core/domain/model_download_boundary.py`. | In-memory pull job | No | Local Ollama daemon | Model setup (Ollama) |
| `GET /models/ollama-pulls/{job_id}` | Poll an Ollama pull job's progress (percent, status, message). | No | No | No | Model setup (Ollama) |
| `POST /models/ollama-pulls/{job_id}/cancel` | Stop a running Ollama pull. | In-memory pull job | No | No | Model setup (Ollama) |
...
| `GET /models/gguf-downloads` | List all GGUF download jobs (newest first) so the UI can re-attach to a download still running after the panel was remounted. | No | No | No | Model setup (llama.cpp) |
...
| `GET /models/recommended-backend` | Which local engine first-run setup should default to, detected from what's installed: a ready llama.cpp model, else a reachable Ollama, else llama.cpp. Read-only probe. | No | No | Local process + Ollama probe | Model setup (auto-detect) |
...
| `POST /models/llama-runtime/embedding` | Switch the built-in engine's search/embedding model to another already-downloaded GGUF, persisting the choice and restarting only the embedding process. Caller should rebuild the index afterwards. 409 if the model is unknown or not downloaded. | Local llama-server embedding process | No | Restarts embedding process | Model setup (llama.cpp) |
| `POST /models/llama-runtime/reset-selection` | Forget the engine's remembered answer/search models so both fall back to the recommended catalog defaults. Called after a full workspace reset (no project left ⇒ no explicit selection); clears the in-memory and on-disk refs and stops any running engine so the next activation starts clean. Best-effort. | Local llama-server processes + runtime state | No | Stops engine, clears persisted refs | Model setup (llama.cpp) |
...
| `POST /models/gguf-import` | Register a GGUF model already on disk (from LM Studio, another llama.cpp setup, or a manual download) so it becomes usable without re-downloading. Links it into the managed model dir (copies only when a link can't be made); validates the GGUF header, extension and size. The user's original file is never modified. | Managed model dir (link/copy) | No | Reads and links a local file | Model setup (llama.cpp) |
...
| `POST /workspaces/{workspace_id}/index/changed` | Incremental re-index: re-embed only files whose content hash changed (drop chunks for changed/removed), so the model sees the latest content without re-indexing the whole repo. | Index status, vector store, manifest, timeline | No | Filesystem plus embedding/vector providers | Workspace setup |
| `GET /workspaces/{workspace_id}/index/changed/preview` | Embed-free count of how many files changed/new/removed since the last index (for the "N files changed, update?" hint and optional auto-update). | No | No | Filesystem read + manifest | Workspace setup |
| `POST /workspaces/{workspace_id}/index/enrich` | Selectively enrich a bounded batch of indexed chunks (docs and context-poor fragments) with a one-line situating note and re-embed them. Off unless `AI_WORKSPACE_CONTEXT_ENRICHMENT` is set (then it returns `enabled=false` and touches nothing). Idempotent and progressive. | Vector store | No | Configured LLM plus embedding/vector providers | Workspace setup |
...
| `GET /workspaces/{workspace_id}/file-activity` | Read-only git ownership and recent-commit history for a single file (`?path=`): total commits, top authors, and the latest commits touching it. Returns empty fields when not a git repo. Never modifies the repository. | No | No | Local `git` (read-only) | File inspector |
...
| `GET /workspaces/{workspace_id}/intelligence/watch/history` | The change-history timeline: every check that found changes, newest first, with commit subjects and any saved one-tap summary. | No | No | Local DB read | Project Intelligence |
...
| `GET /workspaces/{workspace_id}/starter-questions` | Up to four clickable starter questions for the empty Ask composer, built deterministically from the project map (role-ordered, evidence-gated) or a generic fallback; returns the questions plus a `from_map` flag. | No | No | Reads the project map | Project Intelligence |
...
| `POST /workspaces/{workspace_id}/intelligence/investigate/stream` | Same read-only investigation, streamed as SSE — each ReAct step (`event: step`) as it completes, then the answer + sources (`event: final`), so the agent's thinking shows live. | No | No | Local model + read-only tools | Project Intelligence |
