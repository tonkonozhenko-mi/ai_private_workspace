...
from app.core.domain.project_memory import (
    MemoryItem, MemoryKind, select_relevant_memory, format_memory_context,
)
...
def _item(i, kind, text, pinned=False, created="2026-06-01"):
    return MemoryItem(id=i, workspace_id="w1", kind=kind, text=text, source="user", created_at=created, pinned=pinned)
...
def test_select_falls_back_to_recent_when_no_overlap():
    items = [_item("a", MemoryKind.NOTE, "alpha", created="2026-06-01"),
...
def test_handbook_excluded_and_format():
    items = [_item("h", MemoryKind.HANDBOOK, "the handbook"), _item("n", MemoryKind.CORRECTION, "prod is prd", pinned=True)]
...
def test_build_handbook_from_graph():
    graph = ProjectGraph(
        workspace_id="w1",
        entities=[
            ProjectEntity(id="infra:terraform", type=EntityType.INFRA_COMPONENT, name="Terraform", analyzer="terraform"),
            ProjectEntity(id="environment:prod", type=EntityType.ENVIRONMENT, name="prod", analyzer="terraform"),
            ProjectEntity(id="cloud_service:aws-lambda", type=EntityType.CLOUD_SERVICE, name="AWS · Lambda", analyzer="terraform"),
        ],
        findings=[ProjectFinding(id="f1", category="reliability", severity="high", title="No remote state", explanation="...", analyzer="terraform")],
    )
