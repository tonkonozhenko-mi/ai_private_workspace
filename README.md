...
- **Memory & profile** — local, fully-editable project memory + handbook and a cross-project **"About you"** profile, with **review-first** capture. Memory is **provable and self-maintaining**: every note records where the fact came from (file / commit / investigation / manual), a new note that conflicts with an old one offers to **supersede** it instead of piling up contradictions, notes auto-flag as *"check?"* (with the reason) when a file they reference changes, and **guardrails** ("don't assume X") are injected into every answer as hard rules. Recall is hybrid (keyword ∪ semantic) so a paraphrased question still finds the right note, and as memory grows the card surfaces **near-duplicate notes** for a review-first "keep one, retire the rest" merge.
- **Trustworthy answers (anti-hallucination)** — the durable context fed to every answer is allocated by one explicit **budget** (so notes never crowd out retrieved code), the model **abstains honestly** when nothing confident is found instead of inventing file/config details, a deterministic **groundedness check** flags terms the answer states as fact but that aren't in the retrieved text, and obsolete/superseded notes are **never** injected. **Answer modes** (Balanced / Only-from-sources / Deep dive / Explain-with-sources) let you set how strictly a reply must stick to your files — invaluable for keeping small local models honest — and **"Why this answer?"** shows exactly which notes and guardrails shaped each reply. A deterministic, offline **eval harness** (retrieval *and* memory) guards all of this against regressions — measuring not just safety but **helpfulness** and **over-blocking**, so caution never quietly makes the assistant useless. No LLM, no flakiness.
