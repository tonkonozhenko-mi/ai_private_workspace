...
- **A document attached to a question was never actually read.** Dropping a Word file into the chat produced an answer that paraphrased its title and nothing else; asked what was inside, the model described a different file. It looked like it had stopped understanding. It had never been given the document: the browser read the file as if it were plain text, and a .docx is a ZIP, so what was sent was mojibake. The extractor that reads Word, Excel, slides and PDF for the project index was already in the app, and this path had never called it. It does now — and it also reads LibreOffice files (.odt, .ods, .odp) and .rtf. A legacy Word 97 .doc is still refused, but in words you can act on: it names the format and says to save it as .docx, instead of quietly sending noise to the model.

- **The attached document was dropped after the first question.** Attaching a questionnaire and asking about its first question worked; asking about the second sent no document at all, so the answer came from the project index and read like the model had forgotten. Documents now stay attached until you remove them — the chips under the composer are exactly what will be sent, and each has an ×.

- **A question in Cyrillic could not find anything in an attached document.** Which parts of a long document to include was decided by matching words from the question, and the matcher only recognised Latin letters. A question written in Ukrainian produced no words at all, so the choice fell back to the first few thousand characters every time — fine for a first question about the top of a file, useless for anything further down.

- **Answers to a question written in Cyrillic come back in Ukrainian.**
...
- **Saving your roles in Settings did nothing at all.** The panel sends every role in one request, and the request would only accept five of them — a number typed in when there were five roles, which did not move when the sixth (DBA) was added. From that day every save was refused outright, before anything looked at what was in it. Underneath that sat a second fault of the same kind: the server kept a different set of five names, so four of the six roles would have been discarded without a word even had the request got through, and a role the server never returns falls back to its default when Settings reads it back — which is why it looked like the app kept forgetting and reverting to DevOps. The number of roles is now counted rather than written down, in all five places that had their own copy of it, including the one that decides how much of your guidance reaches the model. Guidance written under an older role name is carried over rather than lost, and a skill the server genuinely cannot place is refused with a message that names it.
...
- **Ask for the answer in English and you get English.** A question written in Ukrainian asked for onboarding documentation "all of it in English"; the answer came back in the language of the wiki it had read. The prompt had never mentioned language at all, so the documents decided. There is a rule now, at the end of the prompt where a small model remembers it best, in the order a person would expect: what you asked for in this question wins; failing that, the preference saved in About you; failing that, the language you wrote the question in. Never the language of the files. It survives a typo, too — "має біти англійською" asks just as clearly as the correctly spelled version, and a rule a slip can switch off is not a rule you can rely on.

- **A page of true statements about Terraform is not an answer about your project.** Asked to write onboarding docs from the project's own documentation, the model produced fluent paragraphs — "Terraform is a tool for infrastructure as code", "Kubernetes is an open-source orchestration system" — that came from what it knows rather than from anything indexed. The existing check looks for files that do not exist and could not see this one: it named no files at all. A long answer that points at nothing in your project is now treated as what it is — grounds to answer again with the sources named, and a warning if it happens twice. Short answers are never flagged: naming a file to say "yes, object storage" is pedantry.
...
a conversation — which is the only way it can answer "what did we decide, and
where is that configured?", the question a group exists for. It has the same four
answer styles a single project has. It notices when one of its repositories has
moved on, and offers to catch up rather than doing it behind your back. Its
handbook records what it read, so it can admit when it has fallen behind instead
of quietly ageing.

And it stops guessing. Most of what follows is one rule arriving in the last
places that had not heard it: **not knowing is not the same as knowing there is
nothing.** A decision that was superseded now hands over its replacement rather
than pointing at it. An answer that names a file nobody has is answered again. A
question about a share price is small talk however it scores. And a git call that
timed out now says nothing, instead of reporting a repository with 1,390 commits
as no repository at all.

### Added

- **A group can hold a conversation now — which is the only way it can answer the question it exists for.** "What did we decide about report storage?" and then "and where is that configured in the code?" is one question in two parts: the decision lives in the wiki, the code lives in the repository, and the second half means nothing without the first. The group Ask had no memory of the turn before; every question was the first question. The thread is kept now, the earlier questions steer the search so "that" finds the right files, and "+ New chat" starts a fresh one. A group's conversation belongs to the group, in the same place its notes already live.

- **The group Ask has the same answer styles as a single project.** Balanced, Only from sources, Deep dive, Explain with sources — the same four, with the same effect on how hard the model leans on your files and how readily it declines. In a group the strictness applies to every repository equally: a style decides how the answer is written, never which projects are heard.

- **A group notices when one of its repositories has moved on.** Change a few files in a project and its card in the group says so — "6 files changed since last index" — with a Rescan beside it. Rescan is that project's own rescan, not a group copy of one: everything it writes lands in the project, so switching to it on its own shows the same history it always did. Nothing is stored at the group level, because a group is a view over its members, not a second place to keep their facts. And a repository that has not changed says nothing at all: a screen where everything wears a badge is a screen where a badge means nothing.

- **The group handbook admits when it was written before the news.** The handbook summarises every repository in the group, and it now records which version of each it read. Re-index one of them and the handbook says "Built before the latest changes in Wiki — Regenerate?", on Home and above every group question. It keeps being used in the meantime: an old summary is worth more than none, as long as nobody is pretending it is current. Rebuilding costs real time on the model, so the app offers and you decide — it never quietly rebuilds behind your back. A handbook written before any of this existed says nothing, because not knowing what it read is not the same as knowing it is behind.

- **A group answer says which repositories answered from an old reading of themselves.** If a repository's files have moved past its index, its block under SOURCES BY REPOSITORY says so in one quiet line. Not a banner and not a warning — the answer is exactly as good as what was indexed, and this is how old that reading was. It is the same arithmetic as the badge on Home, asked from the other end, so a repository cannot be behind on one screen and current on the next.

### Changed

- **The group screens were tidied up: less furniture, fewer duplicates, nothing hiding behind anything.** The collapsed-sidebar button was sitting on top of the page title, so "PROJECT GROUP" read "OJECT GROUP". Membership chips appeared on every tab, where on Intelligence they were the identical twin of that tab's filter and did the opposite thing — they now live only on Home, where the group is managed, and the filter looks like a filter. A repository count that the header, the chips and the list below all stated was being stated a fourth time in a card of its own; the week's commits became a badge on the repositories they belong to; a folder that is not a git repository says so in one line instead of three zeros. Intelligence with nothing analyzed yet is one invitation and a button per repository, rather than five sections each finding its own way to say the page is empty, and "Not analyzed" is now a neutral badge instead of a green one — it is a state, not a success. In a group conversation your question is a block of its own, as it is in a single project, and an older turn folds its sources and feedback into one line so the answer you are reading is the thing you are reading. Warnings never fold: honesty is not something you should have to click for. Same answers, same sources, same labels — only the room around them is quieter.

- **Group screens now sit in the same band as the rest of the app, instead of being stretched across the monitor.** A group was the one project view with no width limit: on a large display its title ran to one edge, its buttons to the other, and a metre of nothing sat in between. It now uses the width a single project's dashboard has always used, so the title, the tabs, the cards, the notes and the conversation line up on one axis and end at one edge — a badge beside a heading is beside it. Extra width becomes more repository cards per row rather than wider cards, and a group conversation is capped at the measure a single project's Ask uses, because an answer two thousand pixels wide is one your eye loses its place in.
...
- **A group question about a share price could still be answered from your code.** The measured chit-chat ceiling — how high an unrelated question scores against your own project — was applied in the group as a discount only, never as a raise. It is the bug that was fixed for single projects two releases ago and never carried across; the group now uses the same rule, in the same order, and there is one fewer copy of it to drift.
...
- **A superseded decision now hands over its replacement, instead of pointing at it.** Telling the app a page was out of date turned out to be half a fix: told that ADR-05 had been replaced and shown nothing else, the model described the replacement it had never read — inventing a storage service that appeared in none of its sources. But the page names its successor in the same line ("Superseded by [ADR-08] Report storage v2"), and that successor is sitting in the same index. The app now follows the pointer — a lookup, not a guess: no search, no model — and puts the live decision in front of you, ahead of the dead one. Where the replacement genuinely is not there, the app says which document to look for and the model is told, in as many words, not to describe what it has not seen. This works when you ask one project and when you ask a group of them: in a group each project's pointer is followed in that project's own index, because a wiki's link addresses a page in the wiki, not in whatever repository sits beside it.

- **An answer that names a file which does not exist is now caught and re-tried.** The app checked the answer for invented file paths, but only between backticks — the form the prompt asks for, not the form a model uses when it is making something up. An answer that placed a project's storage configuration in "main.tf in the Terraform directory", in a project with no Terraform in it, sailed through. Plain prose is read now too, a file that any retrieved document merely mentions still counts as read rather than invented, and naming a file that appears nowhere is treated as what it is: grounds to answer again, not a footnote.
...
- **A repository with 1,390 commits is no longer announced as "not a git repository".** While macOS was asking permission for a folder, the group's git call timed out — and the card stated, flatly, that the project had no git history at all. One field was holding two facts: what git answered, and whether git answered. They are separate now, and when the app could not ask, the line simply is not there. Not knowing is not the same as knowing there is nothing; the same rule the answers have followed for two releases now applies to a card reading a number off a subprocess.

- **A group card no longer reports a file as removed when nothing was removed.** One repository was stuck on "1 file changed (1 removed)" — nothing had been deleted, and no amount of rescanning would clear it. The check compared the files on disk against the last saved scan, but the two were being chosen by different rules: the rescan saved every file it could find, while the check only counted the ones the project's own file rules select. Anything in the gap was "removed" on every check, for ever, and the rescan was the very thing writing the mismatch back. Both sides now ask the same question of the same rules, so the count means what it says.

- **Saving a good group answer saved the question as blank.** The composer empties when you send, and the thumbs-up read the question from the composer — so every note the group remembered began "Q: " with nothing after it. The question is kept beside its own answer now.
...
- **The model that wrote the paragraph is named, not spelled out.** "written by Mistral 7B from your files"; the quantisation and the full path are true, and they wait in the tooltip for whoever wants them.
- **A design pass that adds nothing and changes everything.** No feature, no data, no label moved — only the way the app carries itself. One spacing scale (4/8/12/16/24/32/48) instead of numbers decided a component at a time, and a rule for using it: a beat between regions of meaning, a breath inside one. The Intelligence dashboard was a stack with no pauses — chips, attention, questions and tabs arriving as a single paragraph of furniture — and now reads as what it is: a briefing, then the tabs, then the content.
- **Monospace is for what a machine emitted.** The list of decisions on a wiki was a wall of green monospace: titles that people wrote in sentences, set in the font we reserve for paths and commands. Titles are text now; where a document carries its folder to tell it apart ("modules/vpc · README"), the folder stays mono and quiet and the name is text.
- **Air instead of boxes.** A card inside a panel inside a page drew three borders to say one thing. The outer edge earns its keep — it separates the app from the content — and the ones inside it were separating content from content, which space already does.
- **One motion language.** Four tokens, and not one more: one curve, three durations. Tabs settle rather than snap, and the role change — the moment this app is judged on, the same facts re-framed for who you are — now fades rather than jumping. Nothing but opacity and transform is ever animated, and everything stops for anyone who has asked for less motion.
- **Colour, given meanings.** One green was doing five jobs — the action, the link, the running engine, the fact chip, the active tab — and when everything is the accent, nothing is. Five meanings now have five tokens in the same family: what you can do, where you can go, what is well, what is worth a look, and what is merely worth knowing.
...
- **The copy pack from the final review:** the Documents list on a repository is headed "Documents", not "Pages"; a rebuild that finds the same totals says so without inventing a reason for it ("Map rebuilt — same totals: 174 things, 1 risk"); the model that wrote the architecture paragraph is named the way a person would name it ("written by Mistral 7B from your files"), not by its file path; a project is "an infrastructure project", never "a infrastructure project", and never on Terraform twice; a group's cards count in words that agree with their numbers; and a wiki is asked where to start reading "this documentation", not "this repo".
- **Pressing "Re-read the files" re-reads the files.** The map is cached against the files and the app version — and a release that ships under the same version number is, to the cache, the same program. So after an update the map on screen was still the one the old analyzers built, and the button replied "nothing changed" and left it there. Two things were wrong and both are fixed: the analyzers now carry a version of their own, and an explicit click is treated as an instruction rather than a cache miss to be argued with. The cache still spares the machine work nobody asked for; it no longer overrules the person who did.
- **A repository stopped introducing itself as a wiki.** A Terraform monorepo with a README in every module announced itself as "106 pages of documentation" and never mentioned its 938 `.tf` files — because pages happened to be counted first. A project now leads with what it mostly is; documentation inside a repository comes last and is called what it is: documents.
- **One sentence per project, each word once.** The old line read "infrastructure project (Terraform, CI/CD), 6 module(s); infrastructure: Terraform; 4 CI/CD pipeline(s)" — a train of semicolons saying infrastructure twice.
- **A group's cards show what the group is made of.** Four fixed code-shaped numbers meant a group holding a wiki led with "0 services" and never mentioned its 169 documents.
- **The same risk in twenty files is one thing to look at.** Three cards reading "Secrets referenced" — one per workflow — ate three of the five slots on Home, so a project's five most important things were two things and an echo. They are grouped: "Secrets referenced · in 3 places".
- **The prose says whose it is.** "Architecture at a glance" is written by a language model and sat unlabelled among deterministic facts. It now carries its provenance, like every other model-written line in the app.
- **`.agents/skills/…/SKILL.md` is not where a newcomer starts reading.** Files that configure somebody's assistant are no longer offered as a way into the project.
- **A group can see a wiki.** Put a Terraform repository and an exported wiki in one group and the wiki — with a fully built map of 169 pages and 23 decision records — was announced as "Not analyzed yet", while the very same screen offered to open its full map. The aggregate was counting only the things a code repository is made of. Both views now describe a project from one vocabulary, so a group of mixed projects adds up: "938 infrastructure files, 5 environments" and "169 pages, 23 decisions" in the same summary.
- **A group speaks the same language as a project.** Group risks shouted "HIGH" and "MEDIUM" while the single view said "worth a close look" and "leads for a human, not verdicts". A risk does not become a different kind of thing by being seen from a group.
- **"No test files were found" no longer shouts at an infrastructure repository.** There is nothing there to unit-test; the checks that matter are plan and validate in CI. The fact is unchanged, the tone is: worth knowing, not worth answering for.
- **The missing Terraform backend block is read in the light of Terragrunt.** Where Terragrunt is in use it generates those blocks, so their absence from the .tf files is expected rather than a gap — and the app now says where the state actually lives (the `remote_state` block in `terragrunt.hcl`) instead of implying a hole.
- **The names of CI jobs are not technologies.** "Detect Changed Directories", "Terragrunt Apply" and "Terragrunt Plan" were listed under what the project is built with. They are things that run.
- **Group Ask behaves like Ask.** The composer empties when you send, and the starting questions are built from what the group's members are actually made of — "What did we decide, and where is it implemented?" for a group of a wiki and a repository, which is the one question no single member can answer.
- **A repository is no longer told that it is a wiki.** An infrastructure monorepo with a hundred READMEs among a thousand files was handed a wiki's facts: "Pages (106)", six entries all called SKILL, and "nothing links to it" under each README — the exact mirror of the bug where a wiki was told it had no tests. A README does not link to other READMEs and was never meant to. The wiki's facts — areas, links, orphans, staleness — now switch on only when the collection *is* a body of knowledge: documentation is most of what is there, or the pages genuinely cross-reference each other. Everywhere else the documents are simply listed, by path.
- **Documents in a repository are told apart by where they live.** A hundred files named README are a hundred identical rows; the folder is the only thing that distinguishes them, so it comes with the name: "modules/vpc · README".
- **"Worth your attention" now means it.** On a real project the line read "Permissions not configured · Secrets referenced · Matrix strategy detected" — one thing to look at, and two facts about how every CI pipeline in the world works. Only findings that ask for a human reach that line; the rest stay in Risks, reported, where they belong.
- **The map says which of your files it read**, in your words — "read from your Terraform files, GitHub Actions workflows and git history" — instead of listing the nine analyzers that read them.
- **Fourteen tabs no longer wrap onto two rows.** A rich repository has a lot of sections, and a wrapped row of tabs is a wall rather than a place. Your role's lens already knows which come first: six are shown, the rest are one click away under "More", and not one section is lost.

- **The Rebuild button no longer sits beside the role picker.** Standing there, the two looked like a pair — so people changed role, pressed Rebuild, and watched nothing happen. They are unrelated: a role is a lens over the map that is already built; a rebuild re-reads the files. Rebuilding now lives where the reason for it lives — in the line that says how old the map is ("Last analyzed 2 days ago · Re-read the files"), and in the banner that appears when the files have actually changed.
- **Rebuild says what it did.** Rebuilding a map whose files have not changed produces the same map — the right answer, and one that was indistinguishable from a button that does nothing. It now reports the result ("Map rebuilt — nothing changed: the same 174 things, 2 risks, because the files have not changed"), and switching role says what a lens does and does not do: it re-orders what leads and which risks are highlighted, it never changes the facts, and it needs no rebuild.
...
- **Home's closed drawers stopped pretending to be cards.** "AI deep analysis", "Files by area" and "Sources" are a single line with a chevron until you open them; placed in the two-column grid they sat as tall empty boxes beside a full card, and the page read as mostly holes. They now stack full width under the cards that have something to show — and use the whole width when opened, which is what a list of files wanted anyway.
...
- **A wiki page about a test is not a test.** "Payments_Webhook_-_retry_test.html" was filed under Tests because its name contains "test", and spreadsheets attached to pages were filed under Config. A file the scan already recognised as a document is a document.
...
- **The data model, read from your SQL — and a DBA role to read it with.** Tables, columns, primary keys, foreign keys, indexes and views come from the project's own DDL; migrations are listed in the order the tool will really apply them (V10 after V9, not after V1). The map shows tables and how they reference each other, and the risks section names what usually bites: a foreign key with no index, a table with no primary key, a table nothing references. Stated as facts to check, never as verdicts — an append-only log legitimately has no key.
- **Tests, for the tester.** Where the suites live, which frameworks run them, how many cases exist and how many are skipped, whether any CI job actually runs them, and which parts of the code no test file so much as mentions. That last one is deliberately not called coverage: the app never runs your tests, and says so.
- **The rest of the codebase, for the developer.** The map knew only Python. It now reads JavaScript and TypeScript too — package name, framework, entry points, modules and which module imports which — so a React or Node project finally has a map instead of a single "TypeScript application" box.
- **What the system does for its users, for the analyst.** HTTP endpoints (FastAPI, Flask, Express, Spring) become the verbs the system offers, and domain classes become the nouns it speaks in — with the plumbing (repositories, request/response schemas) left out, because a list of `*Response` teaches nobody what a system is for.
- **Where the knowledge is concentrated, for the manager.** Actively-changed files whose history was written by effectively one person, and who those people are. Not an accusation — that is where the project's knowledge lives, and what it would lose if that person were away.
...
- **No role is preselected when you create a project.** DevOps used to be chosen for you, which quietly made everyone a DevOps engineer; now the cards start unset, the question explains what it does, and "just exploring" is a legitimate answer.
...
- **Your actual code is now indexed, whatever it's written in.** The index knew Python, Terraform, YAML and CI files — which meant a TypeScript, Go, Java, Rust or Ruby project was scanned, found nothing it recognised, and could answer nothing about itself. Two dozen languages are now read and cited like any other source, so "where is this defined and what does it do" works on a real codebase.
- **Config beyond YAML and JSON.** TOML, INI, .cfg, .properties, Makefiles, XML and committed .env templates are indexed, and a config chunk carries its key names in the header — so "what linting rules are configured" finds pyproject.toml by the name of the setting rather than hoping the value happens to look like the question.
- **Spreadsheets as data, notebooks as narrative.** A CSV or TSV is indexed in blocks of rows with the header row repeated, cited as `rows 2-21`; a Jupyter notebook is indexed cell by cell, cited as `cell 7`, with outputs left out (they are re-runnable noise, and one plot is megabytes of base64).
- **A project introduces itself by the language it is written in.** A TypeScript repository used to appear as "unknown" — or, worse, as a Python application — because only Python had a code analyzer. The scan now names the dominant language, so the project summary reads "TypeScript application" and the home screen stops looking empty.

### Changed

- **Machine-written files are kept out of the index.** Dependency lockfiles, minified bundles, source maps and compiled output are skipped by name, and any source file with an absurdly long line is refused as generated — one committed bundle could otherwise outweigh an entire codebase and fill answers with unreadable citations. A real `.env` is never indexed (its committed `.env.example` still is), because it holds credentials.
...
- **The setup step no longer quotes a wrong download size.** The "Give your project a local brain" step hard-coded "about 2.5 GB", which didn't match the models actually offered (which can total more). The description now points to the real per-model sizes shown on the cards below instead of a fixed, sometimes-wrong number.

- **The Models screen no longer crashes when you switch the workspace's engine mid-setup.** Guided setup had a code path that ran one more internal step when the workspace flipped from the built-in engine to Ollama than it did before the switch, which crashed the screen (it recovered on its own, but the panel went blank until you left and came back). The step now always runs, so switching engines from the Models screen is smooth.
- **Guided setup follows the engine that's actually running, not a stale selection.** If a workspace was running on the built-in llama.cpp engine but still had an old Ollama model recorded, "Choose & install" could show the Ollama flow — and try to install Ollama models on a machine without Ollama. It now shows the built-in engine's model manager whenever llama.cpp is the live engine, so it never offers models for an engine that isn't installed.
- **A workspace whose answer model can't run now offers a one-click fix onto the model that is running.** If the selected answer model was left in a broken state (e.g. an Ollama pick that never installed) while the engine was happily running a different local model, the Models screen was stuck on "needs setup". It now shows a "Use active answer model" button that switches the workspace onto the running model — the same recovery that already existed for the search model.
...
- **Every feature that writes with the local model now uses the engine that's actually running, so nothing quietly falls back to Ollama.** Beyond the project overview, a few features that don't carry an explicit model choice — "Investigate deeper", the background context enrichment, and the About-you suggestion prompts — asked for the app's configured default engine, which on many setups is Ollama. On a machine running the built-in llama.cpp engine with no Ollama, those calls failed with "Unable to reach Ollama". When no specific model is requested, the app now generates with whichever engine is live (llama.cpp or Ollama), falling back to the configured default only when nothing is active — so these features work on a llama.cpp-only machine like the rest of the app.
...
- **The app now switches to its built-in local engine when Ollama is missing, instead of getting stuck on a dead one.** Answers and search can run on either Ollama or the bundled llama.cpp engine. Previously, if a project was set up with Ollama, the app would always point back at Ollama when it started or when you opened that project — without ever checking whether Ollama was still installed. So after uninstalling or stopping Ollama, every question hit an engine that was no longer there and the app never moved over to llama.cpp on its own. Now, whenever it would otherwise fall back to Ollama, it first checks that Ollama is actually reachable; if it isn't, it brings up the local llama.cpp engine and switches to it, as long as a local model is available. If no local model is downloaded, it stays put and records a clear note that a model needs downloading (or Ollama restarting). The existing safety net in the other direction — falling back from llama.cpp to Ollama when the local engine can't start — is unchanged.
...
- **"How did the AI reach this?" now shows which About-you preferences were applied.** The answer trace previously counted only project-memory notes and map facts, so there was no way to confirm that your cross-project profile actually reached an answer. It now lists the About-you facts that shaped the answer — as a step in the reasoning trace and as their own section alongside memory and sources — with a count on the summary line, so you can see at a glance that a preference like your answer language was picked up.

### Fixed

- **Scanning a project is dramatically faster when it contains large ignored folders.** The scan walked every file on disk — including the giant folders your project already ignores (dependency caches, build output, data dumps) — and only discarded them afterwards. On a repo with a big ignored tree this meant stat-ing and inspecting hundreds of thousands of files to keep a thousand, so the opening "Enumerating files…" step could take a minute or more. The scan now skips ignored folders as it walks, exactly as git does, so it never descends into them. The set of indexed files is identical — only the wasted work is gone (in one real case, a scan that walked ~166,000 files now touches ~1,000). The file-rules preview still shows ignored files, labelled as excluded, so nothing is hidden from you there.
- **The local model's name reads properly instead of a bare slug.** On the llama.cpp engine, the current model showed as its internal id — "qwen3-4b — llama.cpp" — with no sense of which variant it is. Known catalog models now display their real name and quantization, e.g. "Qwen3 4B Instruct · Q4_K_M — llama.cpp", wherever a model is named (the setup card, answer headers, project analysis). Ollama's own short tags and models you imported yourself are unchanged.
- **Answers no longer leak the internal "project handbook" identifier into their text.** When answering broad questions like "what is this project about?", the app draws on an auto-built project summary that's stored under an internal name. Small local models sometimes copied that raw internal name straight into the answer ("… the README.md and `__project_handbook__` document this"). The model is now only ever shown the friendly label "Project handbook", so answers read cleanly, and citing it still counts as a valid, grounded source. Any answer produced before this fix has the raw identifier softened to the friendly label on the way out.
...
- **Project scans now record where their time goes, to diagnose slow first scans.** Some scans spend a surprisingly long time on the opening "Enumerating files…" step even on tiny projects, with no visibility into why. Each scan now writes a single timing line to the log breaking that step into its parts — walking the folder, classifying file types, reading ignore rules, applying filters and detecting technologies — along with how many paths the walk actually visited versus how many were kept. A walk that visits far more paths than it keeps points at a large neighbouring folder being traversed; even timings on a small project point at the real culprit instead of guesswork. No behaviour changes for the user; it's diagnostic output only.
- **The "not in this project" line is calibrated more accurately on small, focused codebases.** Deciding when to answer from your files versus admit the answer isn't there depends on a similarity threshold. That threshold was anchored to one measurement — how alike random pieces of your own project look to the search model — plus a fixed margin. On a small, homogeneous repository (say an infrastructure repo where everything shares the same vocabulary) that anchor sits too high, so real questions were sometimes waved off as "not found" even though the answer was right there. A second anchor now corrects this: at index time the app quietly measures how strongly a handful of deliberately off-topic phrases (a greeting, the weather, simple arithmetic) resemble your project, which is the true level genuine small talk reaches for your specific project and search model. The threshold is held just above that level. It can only ever lower the bar, never raise it, so honest "I don't see that here" answers are unchanged while good questions on tightly-scoped projects get answered more often. The same calibration now applies whether you ask one project or a group of them. It costs a few extra milliseconds during indexing and needs no model of its own.
...
- **A risk shown at the group level now explains itself.** The group's Intelligence listed repeating risks by title and where they occurred, but to understand or fix one you had to open the individual project. Each grouped risk now carries the same plain-language explanation and suggested fix a single project's risk card shows, so the whole portfolio can be triaged in one place.
- **A group's Intelligence now opens the full project view for any repository in it.** The group compared repos at a glance (shared environments, repeating risks, technologies) but couldn't show the deep, per-project analysis. Each repository in the group is now openable in place: one click shows that project's complete Intelligence — risk cards, environment matrix, CI/CD flow, map, and the rest — exactly as if the project were opened on its own, with a step back to the comparison. Comparison across repos on top, full depth on demand underneath, from one screen.
...
- **A gentle first-run orientation so a new user knows where to start.** The first time you open a project, a short guided tour dims the screen and spotlights each part in turn — the projects button (which is tucked away by default), then Home, Intelligence, Models and Settings, finishing on Ask with an invitation to try a first question. You learn where everything is and what to click, and the tour ends pointed at the action rather than a dead end: finishing leaves you in the Ask tab. It runs once, is fully skippable (or step through with the arrow keys), and can be replayed any time from the command palette or a "Take a quick tour" link in Settings → About. The first time you open a section, a one-line caption also explains it, then quietly goes away. Nothing blocks you — the app teaches in place and gets out of the way.
- **Use a model you already have — no re-downloading.** People who already keep GGUF model files locally (from LM Studio, another llama.cpp setup, or a manual download) can now point the app at one and use it straight away, instead of downloading a second copy. In the model manager, "Choose .gguf file…" opens a native picker (or paste the path); the file is checked to be a real model, linked into the app's model folder in place, and appears in the list ready to switch to. Nothing large is copied — a link is used, and it falls back to a copy only when a link can't be made — and your original file is never moved or modified. Deleting the imported model removes only the app's link, never your file.
- **The index can quietly improve itself in the background (opt-in).** Search chunks that lose their bearings when a file is split — documentation cut mid-section, and code fragments with no nearby function or heading to anchor them — are the ones that lead to weak or wrong retrieval. When enabled, the app now walks a small, capped set of exactly those chunks, asks the local model for a one-line "where this sits and what it's about" note, and re-embeds the chunk with that note attached, so it becomes findable by what it actually is. It runs a bounded batch at a time and is safe to repeat: already-improved chunks are skipped, so each pass picks up where the last left off, and any single failure is quietly skipped rather than disturbing the index. Off by default, because it spends a model call per chunk and the product's first duty is to run well on modest hardware; a single setting turns it on for those who want the extra retrieval quality.
...
- **"Investigate deeper" now shows the agent thinking live.** The multi-step investigation used to run silently and drop its whole trace at the end, so a long run looked like a frozen spinner. Each ReAct step — the thought, the tool it reached for, what it found — now streams into the panel as it happens, then the final answer and sources land at the end. Same read-only tools and same result; only now you can watch it work. (A new streamed endpoint alongside the existing one; the non-streaming path is unchanged.)
- **Config files are now findable by their key names.** Asking "how is the content security policy set?" missed the file that configures it, because the file only spells it `csp` — a short value buried under a nested `security` key that neither meaning-based nor keyword search could reach. When a `.json`/`.yaml` chunk is indexed, its config keys (at any nesting depth) are now listed in the chunk's searchable header, so keyword search hits by key name — `csp`, `security`, and the rest surface the file even when the value itself is a single token. Deterministic and regex-based (config chunks are usually fragments, so it never tries to fully parse them); the keys go in the header only, so the meaning-based vector still embeds the clean file content. This lifts the whole "how is X configured?" class on any config file, not just one case.
- **Deep-dive answers can be constrained to a strict cited-answer shape (experimental, off by default).** When the local engine can constrain its output to a schema (the built-in llama.cpp engine, and newer Ollama), Deep-dive mode can ask the model for a rigid `{answer_md, citations:[{path, quote}]}` object instead of free text. The grammar makes it impossible for the prose to trail off into a raw `source_path:` list, and turns citations into structured data rather than something parsed out of prose. Gated to Deep-dive only and off unless explicitly enabled (constraining a small model to a rigid schema can flatten its writing, so it wants A/B testing first); parsing is fail-open, so a malformed or plain-text answer is shown as-is.
...
- **Quote-check no longer flags generated command examples.** The `quote_not_in_sources` grounding check treated a fenced shell block in a how-to answer (e.g. "run ```bash terraform init && terraform apply```") as a verbatim quote that must appear in the files — a false positive, since that's a generated example, not a citation. On a how-to question, fenced blocks tagged as shell/command languages (`bash`, `sh`, `console`, `powershell`, …) are now skipped. Non-shell blocks (config snippets like `hcl`) and quotes on non-how-to questions are still verified.
- **The corrective retrieval pass now asks for *different* search terms.** CRAG-lite's one-shot corrective retrieval used the same query-rewrite prompt as the normal path, which — once the LLM rewrite is enabled by default — would likely reproduce the query that just failed. The corrective pass now uses a distinct prompt ("a first search found nothing relevant; produce a *different* query — synonyms, alternate identifiers, related concepts"), so the retry actually explores new terms instead of wasting the extra model call.

- **"Investigate deeper" on an abstained answer now starts the agent immediately.** The handoff button previously just opened the trace panel, where you had to click "Investigate deeper" a second time — the label promised a run it didn't do. Opening from the abstain handoff now auto-runs the investigation once (a new `autoRun` prop on the trace panel); opening the same panel via "How did the AI reach this?" still just shows the trace, no run.
- **Ask makes one corrective retrieval pass instead of giving up (CRAG-lite).** Most RAG failures are retrieval, not generation — the right file just didn't rank. Now, when Ask is about to abstain on a project-looking question (best match below the relevance floor), it makes one bounded corrective pass: a forced LLM rewrite of the search query, re-retrieve, and continue only if the new best match clears the floor — otherwise it abstains honestly as before. On the non-streaming path there's also a post-answer trigger: if the finished answer has hard grounding warnings (cited no retrieved file, or asserted a project term that isn't in the context), Ask retries retrieval + regenerates once, and keeps the new answer only if it *strictly* improves grounding. Bounded to a single extra retrieval (+ at most one regeneration), gated on project-looking questions so chit-chat never pays the cost, and fully fail-open. Streaming applies only the pre-generation correction (you can't rewind tokens already streamed).
- **Abstained project questions now offer "Investigate deeper".** When Ask declines because nothing in the project matched, the answer surfaces an inline **🔍 Investigate deeper** action that hands off to the ReAct agent (which can search code, read files and follow the map) — a real path forward instead of a dead end. Shown only when the question actually looked project-specific, so it never appears on general chit-chat.
- **Answers that quote text now have that quote verified against the sources.** A new deterministic check flags `quote_not_in_sources`: any verbatim-looking span in the answer (a code block, a double-quoted phrase, or a long inline-code span over ~20 chars) that doesn't appear in the retrieved files, whitespace-normalized so reflowed indentation still matches. A quote is a claim of exactness — this catches a paraphrase dressed as a quote, or an invented snippet, which the existing identifier/path checks miss. Read-only (it flags, never edits); surfaces in the answer-trace grounding notes.
- **Retrieved chunks are expanded to their neighbours before answering (parent-document / small-to-big).** We search on small 1500-char chunks for precise matching, but a matched chunk was also *all* the model saw — e.g. the middle of a function without its signature or imports, a classic hallucination source. After ranking (MMR/rerank) and before the context budget is applied, each retrieved chunk now grows with its ±1 neighbours in the same file; overlapping expansions merge into one contiguous span (deduped per file), the first chunk keeps its provenance header and the rest are joined header-stripped. Fully deterministic and fail-open (any lookup failure keeps the plain retrieved chunk); the per-file span is char-capped and `fit_context_results` stays the final overflow guard. Added `get_source_chunks(workspace_id, source_path)` to the vector-store port and all three adapters (in-memory, SQLite, Qdrant; Qdrant's payload now carries `chunk_index`).
- **Small projects skip retrieval and answer from the whole codebase.** On a small repo (the typical first thing a new user points the app at) the entire project fits the model's window, so retrieval only adds the risk of missing the right file. When every indexed chunk provably fits — a deterministic gate on chunk count vs the window, using a generous per-chunk ceiling so it needs no content read — Ask now feeds *all* files wholesale instead of retrieving, with citations intact. `fit_context_results` still guards against overflow. Driven by the index manifest (which enumerates every indexed file); off when no manifest is wired.
- **The project handbook is indexed as a pseudo-document, so "what is this project about" becomes retrievable.** Broad questions used to depend on the right source file happening to rank. The deterministic handbook (built from the project map) is now embedded at index time as a synthetic `__project_handbook__` document, so a summary of the whole project surfaces through normal retrieval. It's refreshed on incremental re-index only when its text changes, and is fully optional — skipped when no map exists, and indexing never fails if the handbook can't be built.
...
- **Answer modes now change what's retrieved, not just the prompt wording.** The Ask "Mode" options (Balanced, Only from sources, Deep dive, Explain with sources) previously differed only by one steering sentence, which a small local model barely acts on — so all four produced near-identical answers on easy questions. Each mode now also tunes retrieval: **Deep dive** pulls a wider context (≈2× the chunk target) and lowers the abstention floor so it considers more marginal matches; **Only from sources** raises the abstention floor so it declines honestly when nothing is a confident match (its prompt clause already forbids outside knowledge); **Balanced** and **Explain with sources** keep baseline retrieval (Explain differs only in how it presents its reasoning). The tuning is pure/deterministic and applies to both engines.
- **Investigator agent is far more reliable on small local models — and works the same on llama.cpp and Ollama.** The "Investigate deeper" agent used to burn its whole step budget on malformed replies (`(format)` dead-steps) on a 7B model, then give up. Several fixes, all engine-agnostic: (1) **format failures now have their own retry budget**, separate from the tool-step budget — a model that fumbles the reply format a few times still gets its full allowance of real investigative steps; (2) **the JSON grammar now constrains the tool name to an enum of the actual tools** (+ `""`), so a schema-honouring engine literally cannot emit a blank or hallucinated tool — and this now applies to **Ollama too**, whose structured `format` field is filled from the same schema (with a graceful fallback for older Ollama that returns 400); (3) **tools that need a project map (graph query, CI triggers) are hidden until a map exists**, removing the "I need to build the project map…" dead-end loop; (4) a parse failure is fed back to the model as a plain correction instead of a fake action. Net: usable agent runs on any model and either engine, not just ones that honour a grammar.
- **Clearer answer-trace panel.** In "How the AI reached this": several retrieved chunks of the *same* file are now distinguished by line number or match score (instead of looking like duplicate rows); an "Investigate deeper" pass that didn't converge is labelled **tentative** and no longer implies it refines the quick answer; and format-retry steps are dimmed so the real reasoning reads clearly.
- **Markdown parser extracted and covered by tests.** The hand-rolled answer parser (`parseMarkdownBlocks`, inline tokenizer, `safeLinkHref`, `escapeHtml`) moved from inside `AskWorkspace.tsx` into a small pure module `src/lib/markdown.ts`, with a snapshot + fuzz corpus (`src/lib/markdown.test.ts`). The corpus locks in the supported subset, adversarial inputs (unclosed code fence, ragged table, nested emphasis, dangerous link schemes), and a fuzz guarantee that no input — however malformed — can make the parser throw and blank the chat. This is the safety net that justifies owning a parser instead of pulling `react-markdown` and its transitive tree into the render path of untrusted text. Runnable now via `npm run test:markdown` (no vitest required); the same test file drops into a vitest runner once one exists.
- **Abstention threshold now calibrates to the embedding model instead of a hardcoded 0.38.** Cosine scores aren't comparable across embedders — "unrelated" text sits near 0.1 for one model and 0.5 for another — so a single constant caused false "not in the project" answers (threshold too high) or confident answers from noise (too low) whenever the model changed. At index time we now sample random chunk pairs (their similarity *is* the model's noise floor) and store `p95 + margin`, clamped to a sane band, as this index's relevance floor; Ask uses it automatically. No labels and no LLM — it recalibrates itself whenever the embedder changes. Precedence: explicit env override > fake-embedding constant > calibrated floor > the old 0.38 default (kept as a fallback for indexes too small to sample, or built before this existed). Incremental re-indexes keep the previous floor when too few chunks changed to resample.
...
- **Ask no longer wrongly refuses to answer on-topic questions (calibrated floor capped).** The per-embedding-model abstention floor is sampled from random chunk pairs as a "noise floor", but in a topically-focused repo those pairs aren't truly unrelated (they share the project's vocabulary), so the sampled value could sit *above* real query↔chunk match scores and make Balanced answer "I don't have that information" for questions whose answer is right there in the files. The calibrated floor is now treated as **permissive-only**: it can lower the abstention bar for embedders whose matches score low, but is capped at the historic default (0.38), which is proven not to over-abstain. (Deep dive still lowers it further via its mode multiplier.)
...
- **Faster indexing over Ollama — batched embeddings.** The Ollama embedding provider now embeds a whole batch of chunks in one `/api/embed` request instead of one HTTP round-trip per chunk (the indexer already groups by 64, so it's ~64× fewer requests). Falls back to per-text embedding on older Ollama builds that lack the batch endpoint.
...
- **"Investigate deeper" — see the agent's real ReAct trace.** Inside the trace panel, a single-project answer now offers an opt-in button that runs the investigator agent live and replaces the derived retrieval summary with its true step-by-step transcript: each step's thought, the tool it called with its input, and the observation it got back, ending in the agent's conclusion. Nothing is fabricated — the derived steps are shown until you explicitly ask for the deeper, real trace.
...
- **Negative memory: project guardrails.** A new memory kind — `guardrail` ("frontend must not run shell commands", "don't assume Qdrant is enabled unless the config says so") — that is injected into *every* answer as a hard rule, regardless of keyword or semantic overlap, because a constraint has to apply even when the question doesn't mention it. Guardrails are kept out of ordinary fact recall (so they're never double-counted) and obsolete ones are suppressed. A cheap, high-leverage guardrail layer for weak local models. `core.domain.project_memory.format_guardrails`.
- **Semantic-noise floor on retrieval.** Parallel memory retrieval now requires a minimum cosine similarity for a note to enter as a *semantic* candidate, so a weak embedder can't drag in unrelated notes; keyword candidates are kept regardless (they have lexical grounding). Plus new eval cases: guardrails always injected / excluded from recall, and the semantic noise floor.

- **Parallel memory retrieval (keyword ∪ semantic).** Memory recall used to take keyword hits first and only re-rank them by embedding, so a note that means the same thing in different words (no shared tokens) could never become a candidate. Now, when an embedder is present, candidates are drawn from *both* keyword recall and the top semantic matches over the eligible notes, unioned, then re-ranked — so a paraphrased question still surfaces the right note. Without an embedder it stays pure keyword + pin + recency; best-effort, so it never fails an answer. `compose_project_context._select_memory`.
- **Stale notes say why.** When the Watcher flags a note stale because a file it references changed, it now records *which* file (`stale_reason`, e.g. "A file it references changed: app/main.tf") and the memory card shows it in the "check?" hint — so the prompt to re-confirm is explainable instead of mysterious. The reason is cleared when the note is confirmed. `memories_referencing_paths_with_evidence`.
- **Hardened memory-quality eval.** The deterministic memory evaluator now covers real scenarios rather than just "did it find the current note": paraphrase recall (semantic), conflicting notes kept distinct, budget pressure (pinned wins the one slot), handbook/obsolete leak, and a regression snapshot pinning the exact selection + order. `score_memory_case` accepts a pluggable selector so paraphrase cases run through the real parallel-retrieval path.

- **Memory UI: contradiction prompt + confidence provenance.** Saving a note in the Project memory card now first checks (deterministically, no LLM) whether it likely contradicts or replaces existing notes; if so, it shows them inline and lets you **Replace this** (retire the old note via supersede) or **Keep both** — so corrections cleanly retire what they correct instead of piling up conflicting memory. Each recorded note also shows where its confidence came from ("you recorded this", "captured automatically", "adjusted from your ratings") instead of an opaque number. Surfaces the supersede/contradiction backend and the explainable-confidence field already shipped.
- **Memory-quality eval harness.** A deterministic, offline evaluator for the *memory* layer (sibling of the retrieval eval): each golden case declares the notes that should be recalled for a question and the ones that must not (an obsolete/superseded note, or a known-irrelevant one), and scoring runs the real memory selector to measure relevant-recall, stale-usage (did a flagged note sneak in), and leak-rate (did an obsolete/forbidden note surface). No LLM, no flakiness — a stable regression guard answering "does memory help or hurt" as ranking changes. `core.domain.memory_eval`.
- **Memory can supersede and detects contradictions.** When a note replaces an earlier one, it now carries a `supersedes` link and the old note is automatically retired (marked obsolete — kept for history, never recalled), so the project keeps one current answer instead of two conflicting ones. And before a note is saved, a deterministic check (`POST /memory/contradictions`) returns the existing active notes it likely contradicts or replaces — token-overlap on the same subject plus a replacement/negation marker (or a correction), no LLM — so the user can choose to supersede the stale note rather than silently pile up conflicting memory. This is the core fix for the "more memory makes answers worse" trap. `core.domain.project_memory.contradiction_candidates`, `FindContradictionsUseCase`.
...
- **Memory auto-staling: notes flag themselves when a referenced file changes.** When the Watcher records what changed (e.g. on app open), any memory whose text references a changed file (by name, like `main.tf`) is flagged **stale**. Stale notes are *not* deleted or hidden — they may still be true — they're just down-weighted in ranking and shown with a "check?" badge and a **Still correct** button so you can confirm (clears the flag), edit, or delete. This closes the loop "project changed → suspect knowledge," so the AI's memory tracks reality instead of silently drifting. Deterministic file-reference matcher; new `POST /workspaces/{id}/memory/{item_id}/stale`.
- **Memory has a lifecycle (confidence + obsolete + recency).** Project memory items now carry a `confidence` (0–1) and a `status` (active/obsolete). Obsolete items stay listed for history but are **never** injected into prompts — so when the project changes, stale notes can't poison answers (the "more memory makes it worse" trap). Among active items, the keyword-overlap score is now weighted by confidence and a gentle recency decay (90-day half-life; pinned items don't decay), so a confident recent note outranks an old uncertain one. New `POST /workspaces/{id}/memory/{item_id}/status`. Backward-compatible: existing items default to active, confidence 1.0.
- **Memory captures the "why": architecture decisions and incident fixes.** Two new project-memory types — `architecture_decision` ("we use terragrunt because each env is a separate AWS account") and `incident_solution` ("the prod OOM was fixed by raising the worker memory limit") — let the team record the rationale and hard-won fixes that a model (or a new teammate) can't recover from the code alone. They flow through the same retrieval and are labelled distinctly in the prompt ("Architecture decision (why)", "Past incident fix") so the model treats them as authoritative context. Selectable in the Project memory card.
- **Groundedness check on grounded answers.** Extends the hallucinated-citation guard: besides flagging cited file *paths* that weren't retrieved, Ask now flags concrete project-specific *terms* the answer states as fact — backticked identifiers and values like env names, config keys or service names — that appear nowhere in the retrieved file text. These are exactly the fabrications small models slip in. Deterministic (no model, no embedder), conservative (backticked prose and vague words are ignored), and shown as a non-blocking "verify these" note. `core.domain.rag_answer_evaluator.find_ungrounded_terms`.
- **Honest abstention on project questions.** When a question looks project-specific (mentions files, services, config, deployment, environments…) but retrieval finds nothing confident, Ask no longer answers it as ungrounded "general conversation" — the prompt now instructs the model to say it couldn't find anything relevant in the indexed project and suggest rephrasing or re-indexing, and a non-blocking note tells the user the answer is general-knowledge, not from their project. A deterministic `looks_project_specific` heuristic distinguishes these from chit-chat. This directly cuts the most common small-model hallucination: inventing file/config details that were never retrieved.
...
- **Project handbook moved from Home into Intelligence.** The auto-generated handbook (working memory derived from the project map) no longer sits on the Home memory card — it now lives in the Project Intelligence tab, next to the map it's built from. Home's memory card stays focused on the notes and corrections you actually write; the handbook is gated on having a map and reads as part of the analysis it summarizes. Same behaviour (Generate / Regenerate / View, fed into every Ask and Investigate as background) — just a better home for it.
