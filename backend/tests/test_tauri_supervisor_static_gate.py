...
def test_tauri_supervisor_static_gate_contract() -> None:
    ...
    assert "backend-start-disabled" in item_ids
    assert "no-process-api" in item_ids
    assert any("read-only" in rule.lower() for rule in payload["safety_rules"])
    assert any("scan" in rule.lower() and "model downloads" in rule.lower() for rule in payload["safety_rules"])
