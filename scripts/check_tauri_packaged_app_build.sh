...
check_contains "frontend/package.json" '"allowScripts"' "npm allowScripts policy is present"
check_contains "frontend/package.json" '"esbuild": true' "esbuild postinstall is explicitly approved"
check_contains "frontend/package.json" '"fsevents": true' "fsevents optional install script is explicitly approved"
check_contains "frontend/src-tauri/tauri.conf.json" '"active": true' "Tauri bundling is enabled for packaged app smoke"
check_contains "frontend/src-tauri/tauri.conf.json" '"withGlobalTauri": true' "Tauri global invoke bridge is enabled for packaged app frontend bootstrap"
...
check_contains "frontend/src-tauri/src/lib.rs" "../Resources/frozen-backend-runtime/AI_PRIVATE_WORKSPACE_FROZEN_RUNTIME_MANIFEST.json" "Tauri packaged app checks direct app Resources runtime path"
check_contains "frontend/src-tauri/src/lib.rs" "find_frozen_runtime_manifests_under" "Tauri packaged app recursively searches bundled Resources for runtime manifest"
check_contains "frontend/src-tauri/src/lib.rs" "desktop-supervisor.log" "Tauri packaged app writes supervisor diagnostics before backend logs exist"
