...
| `GET /runtime/first-launch-readiness` | Return read-only desktop first-launch checklist for backend, workspace data, local models, search store, and macOS launcher readiness. | No | No | Read-only runtime settings and SQLite diagnostics | Models desktop setup |
